	...
	public List<String> add(List<OrderDetailMO> mos) throws LittleCatException
	{
		...
		for (OrderDetailMO mo : mos)
		{
			...
			try
			{
				...
				batchParam.add(new Object[] { mo.getId(), mo.getOrderId(), mo.getBuyType().name(), mo.getResId(), mo.getGoodsId(),mo.getTuanZhangId(), mo.getPrice(), mo.getGoodsNum(), mo.getGoodsName(), mainImgData });
			}
		...
		String sql = "insert into " + TABLE_NAME + "(id,orderId,buyType,resId,goodsId,tuanZhangId,price,goodsNum,goodsName,goodsMainImgData) values(?,?,?,?,?,?,?,?,?,?)";
